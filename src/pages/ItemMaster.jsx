...
const ItemMaster = () => {
    ...
    const handleAddSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setNewItem({ barcode: '', itemName: '', group: '', unit: '' });
    ...
    const handleExport = () => {
        ...
        // Define Header
        const headers = ['Barcode', 'Item Name', 'Group', 'Unit'];

        // Map Data
        const csvContent = items.map(item =>
            [item.barcode, item.itemName, item.group, item.unit].map(field =>
        ...
        if (link.download !== undefined) {
            ...
            link.setAttribute('download', `ItemMaster_Export_${new Date().toISOString().slice(0, 10)}.csv`);
    ...
    const handleFileUpload = async (e) => {
        ...
        reader.onload = async (evt) => {
            ...
            const data = lines.slice(1).map(row => {
                ...
                return {
                    barcode: values[0]?.trim(), // Backend will generate if empty
                    itemName: values[1]?.trim(),
                    group: values[2]?.trim() || 'General',
                    unit: values[3]?.trim() || 'Pcs'
                };
            }).filter(i => i.itemName); // Only require Item Name now

            if (data.length === 0) {
                alert("No valid data found or all rows were empty. Please check your CSV format (Barcode, Name, Group, Unit).");
                return;
            }
                    ...
                    <div className={`p-4 rounded-xl border animate-fadeIn relative ${isEditing ? 'bg-yellow-50 border-yellow-200' : 'bg-blue-50 border-blue-100'}`}>
                        ...
                        <form onSubmit={handleAddSubmit} className="grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
                            ...
                            <div className="md:col-span-5 flex justify-end mt-2">
                                <button type="submit" className={`${isEditing ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-blue-600 hover:bg-blue-700'} text-white px-4 py-2 rounded-lg text-sm font-medium`}>
                                    <Save className="h-4 w-4 inline mr-1" /> {isEditing ? 'Update Item' : 'Save Item'}
                                </button>
                            </div>
                        </form>
                    </div>
